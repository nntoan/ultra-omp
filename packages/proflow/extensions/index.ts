...
export default function agentSkillsExtension(pi: ExtensionAPI) {
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      pi.sendMessage({ customType: "proflow", content: `${PREAMBLE}\n\n${body}`, display: true }, { deliverAs: "nextTurn", triggerTurn: false });
    } catch (error) {
      pi.logger.warn(`proflow: unable to load using-agent-skills: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
}
