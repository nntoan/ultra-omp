...
1. Require a spec. Look for it new-first: any spec under docs/spec/ (`docs/spec/<feature>/SPEC.md`, nested module specs). Only if none is there, fall back to the legacy known paths: SPEC.md at the repo root, docs/SPEC.md, or a file under spec/. A README or arbitrary doc does NOT count. If the only spec you find is at a legacy path, note it and offer to migrate it to docs/spec/ — do not auto-migrate. If none exists, stop and tell the user to run /spec first — do not invent requirements.
2. Establish a clean baseline. Run `git status --porcelain`. If there are uncommitted changes outside the expected planning artifacts (docs/spec/**, SPEC.md, docs/SPEC.md, spec/*, tasks/plan.md, tasks/todo.md), stop and ask the user to commit, stash, or confirm how to handle them. Autonomous per-task commits must not absorb unrelated local work, or the clean-rollback guarantee breaks.
