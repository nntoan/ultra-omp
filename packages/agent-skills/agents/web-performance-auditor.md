...
- **Invoke via:** `/webperf` (dedicated performance audit command). Not included in `/ship` fan-out because it applies only to web applications.
- **Do not invoke from another persona.** Deeper performance passes are initiated by the user or a slash command.
