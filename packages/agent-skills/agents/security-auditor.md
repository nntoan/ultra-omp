...
- **Invoke via:** `/ship` (parallel fan-out alongside `code-reviewer` and `test-engineer`), or a security-focused command.
- **Do not invoke from another persona.** If a deeper security pass is warranted, the user or a slash command initiates it; orchestration belongs to commands, not personas.
